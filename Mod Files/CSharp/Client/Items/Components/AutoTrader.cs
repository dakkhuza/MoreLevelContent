...
namespace MoreLevelContent.Items
{
    ...
    internal partial class SimpleStore : Powered, IServerSerializable, IClientSerializable
    {
        ...
        {
            SimpleStoreState newState = (SimpleStoreState)msg.ReadByte();
            ...
            if ((user != null && user != Character.Controlled) || State != SimpleStoreState.Stopped)
            {
                this.amountToFabricate = amountToFabricate;
            }
            this.amountRemaining = amountRemaining;
            if (newState == SimpleStoreState.Stopped || recipeHash == 0)
            {
                CancelFabricating();
            }
            else if (newState == SimpleStoreState.Active || newState == SimpleStoreState.Paused)
            {
